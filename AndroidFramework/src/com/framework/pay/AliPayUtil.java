...
//	}
	/**
	 * 支付宝支付   只是做了结果的数据解密和调用本地支付宝客户端的操作，
	 * @param info  支付信息，已经加密好了的，本方法没有对该数据进行加密操作
	 */
...
/**
	...
	public class PartnerConfig {
		...
		/*
		 * public static final String PARTNER = ""; // 商户收款的支付宝账号 public static
		 * final String SELLER = ""; //
		 * 商户（RSA）私钥(注意一定要转PKCS8格式，否则在Android4.0及以上系统会支付失败) public static final
		 * String RSA_PRIVATE = ""; //
		 * 支付宝（RSA）公钥用签约支付宝账号登录ms.alipay.com后，在密钥管理页面获取。 public static final
...
//		public static final String RSA_PRIVATE = "";
	...
	private Handler fss = new Handler() {
		public void handleMessage(Message msg) {
			if(mspHelper == null)
				mspHelper = new MobileSecurePayHelper(mActivity);
			boolean isMobile_spExist = mspHelper.detectMobile_sp();
			if (!isMobile_spExist)
				return;
/**			// 根据订单信息开始进行支付    */
...
//				String orderInfo = getOrderInfo(_cion);
//				// 这里根据签名方式对订单信息进行签名
...
//	}
	...
	public static class BaseHelper {
		...
		public static void showDialog(Activity context, String strTitle,
			...
			DialogTools.createDialog(context, icon, strTitle, strText, "确定", null);
		}
...
//		}
		...
		public static ProgressDialog showProgress(Context context,
			...
			dialog.setOnCancelListener(new AliPayUtil.AlixOnCancelListener(
					(Activity) context));
			dialog.show();
			return dialog;
		}
	...
	public class MobileSecurePayer {
		...
		public boolean pay(final String strOrderInfo, final Handler callback,
			...
			if (mAlixPay == null) {
				...
				mActivity.getApplicationContext().bindService(
						new Intent(IAlixPay.class.getName()),
		...
		private IRemoteServiceCallback mCallback = new IRemoteServiceCallback.Stub() {
			...
			@Override
			public void payEnd(boolean arg0, String arg1)
	...
	public class MobileSecurePayHelper {
		...
		public boolean detectMobile_sp() {
			...
			if (!isMobile_spExist) {
				AssetsUtil assets = new AssetsUtil(mContext);
				assets.installapk("alipay.apk");
			}
			return isMobile_spExist;
		}
		// 安装支付宝安全支付服务
		...
		public String checkNewUpdate() {
			String url = null;
			try {
				// JSONObject resp = sendCheckNewUpdate
	...
	public class NetworkManager {
		...
		private void detectProxy() {
			...
			if (ni != null && ni.isAvailable()
				...
				if (proxyHost != null) {
					final InetSocketAddress sa = new InetSocketAddress(
							proxyHost, port);
		...
		public String SendAndWaitResponse(String strReqData, String strUrl) {
			...
			try {
				...
				if (mProxy != null) {
					httpConnect = (HttpURLConnection) url
		...
		public boolean urlDownloadToFile(Context context, String strurl,
				String path) {
			boolean bRet = false;
			//
			detectProxy();
